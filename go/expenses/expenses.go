...
func CategoryExpenses(in []Record, p DaysPeriod, c string) (float64, error) {
	...
	myCategory := Filter(in, ByCategory(c))
	myDaysCategory := Filter(myCategory, ByDaysPeriod(p))
	if myCategory != nil{
		for _, val := range myDaysCategory{
			totalAmount1 += val.Amount
		}
		return totalAmount1, nil
	}else{
